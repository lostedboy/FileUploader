...
//use FM\ElFinderPHP\D
use FM\ElFinderPHP\Utils\Encoder;
...
class ElFinder
{
     ...
     */
    public function __construct($opts, Encoder $encoder = null)
    {
        if (session_id() == '') {
            session_start();
        }
     ...
     * @param $cmd
     * @param $args
     * @param bool $needTransliteration
     * @return array
     */
    public function exec($cmd, $args, $needTransliteration = false)
    {
        if($needTransliteration AND empty($this->encoder)){
            throw new \Exception('Encoder to transliterate is not set');
        }
        ...
        $result = $this->$cmd($args, $needTransliteration);
     ...
     * @author Dmitry (dio) Levashov
     **/
    protected function mkdir($args, $needTransliteration)
    {
        $target = $args['target'];
        $name = $args['name'];

        if($needTransliteration){
            $name = $this->encoder->transliterate($name);
        }
     ...
     * @author Dmitry (dio) Levashov
     **/
    protected function mkfile($args, $needTransliteration)
    {
        $target = $args['target'];
        $name = $args['name'];
     ...
     * @author Dmitry (dio) Levashov
     **/
    protected function rename($args, $needTransliteration)
    {
        $target = $args['target'];
        $name = $args['name'];
     ...
     * @author Dmitry (dio) Levashov
     **/
    protected function upload($args, $needTransliteration)
    {
        $target = $args['target'];
        $volume = $this->volume($target);
        ...
        if (empty($files)) {
            return array('error' => $this->error(self::ERROR_UPLOAD, self::ERROR_UPLOAD_NO_FILES), 'header' => $this->header);
        }

        if (!$volume) {
            return array('error' => $this->error(self::ERROR_UPLOAD, self::ERROR_TRGDIR_NOT_FOUND, '#' . $target), 'header' => $this->header);
        }
